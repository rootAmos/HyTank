"""
@File    :   LNG_properties_MendezRamos.py
@Date    :   2023/07/26
@Author  :   Eytan Adler
@Description : LNG property surrogate models from Eugina Mendez Ramos's thesis.
               While this is no longer used directly by the LNG boil off model,
...
def gng_P(rho, T, deriv=False):
    """
    Compute pressure of LNG gas using the ideal gas law.

    Parameters
    ----------
    rho : float or numpy array
        LNG density (kg/m^3); rho and T must be the same shape if they're both arrays
    T : float or numpy array
        LNG temperature (K); rho and T must be the same shape if they're both arrays
    deriv : bool, optional
    ...
    float or numpy array
        Pressure of gaseous natural gas (Pa) or the derivative with respect to rho
    ...
    if deriv:
        return (
            T * UNIVERSAL_GAS_CONST / MOLEC_WEIGHT_LNG + rho * 0,  # rho * 0 is to make sure shape is correct
...
def gng_rho(P, T, deriv=False):
    """
    Compute density of LNG gas using the ideal gas law.

    Parameters
    ----------
    P : float or numpy array
        LNG pressure (Pa); P and T must be the same shape if they're both arrays
    T : float or numpy array
        LNG temperature (K); P and T must be the same shape if they're both arrays
    deriv : bool, optional
    ...
    -------
    float or numpy array
        Density of gaseous natural gas (kg/m^3) or the derivative with respect to P if deriv is set to True
    float or numpy array
...
def gng_cv(P, T, deriv=False):
    """
    Compute specific heat of LNG gas at constant volume.
    ...
    -------
    float or numpy array
        Specific heat at constant volume of gaseous natural gas (J/(kg-K)) or
    ...
    if deriv:
        ...
        cv_P += -3.65980723192549 * LNG_PMPa
        cv_T += -3.65980723192549 * LNG_T
    ...
    LNG = np.tanh((-4.86826687998567 + -2.62940079620686 * P_MPa + 0.216404289884235 * T))
...
def gng_cp(P, T, deriv=False):
    """
    Compute specific heat of LNG gas at constant pressure.
    This fit is from Eugina Mendez Ramos's thesis (http://hdl.handle.net/1853/64797).
    It is based on data collected at pressures between 100-750 kPa and 20-150 K.
    See page 100 of the thesis for more details on the data source and fitting process.
    ...
    -------
    float or numpy array
        Specific heat at constant pressure of gaseous natural gas (J/(kg-K)) or
        the derivative with respect to P if deriv is set to True
    float or numpy array
        If deriv is set to True, the derivative of specific heat with respect
        to temperature
    ...
    if deriv:
        ...
        cp_P += -521.915978427339 * LNG_PMPa
        cp_T += -521.915978427339 * LNG_T
    ...
    LNG = np.tanh((-0.400739830511314 + -0.0140493655427582 * P_MPa + -0.000581742517018706 * T))
...
def gng_u(P, T, deriv=False):
    """
    Compute internal energy of LNG gas.
    This fit is from Eugina Mendez Ramos's thesis (http://hdl.handle.net/1853/64797).
    It is based on data collected at pressures between 100-750 kPa and 20-150 K.
    See page 100 of the thesis for more details on the data source and fitting process.
    ...
    -------
    float or numpy array
        Internal energy of gaseous natural gas (J/kg) or
        the derivative with respect to P if deriv is set to True
    float or numpy array
        If deriv is set to True, the derivative of specific heat with respect
        to temperature
    ...
    if deriv:
        ...
        u_P += 399.518468943879 * LNG_PMPa
        u_T += 399.518468943879 * LNG_T
    ...
    LNG = np.tanh((2.05471470504367 + 2.13093505892378 * P_MPa + -0.119572061260962 * T))
...
def gng_h(P, T, deriv=False):
    ...
    -------
    float or numpy array
       Enthalpy of gaseous natural gas (J/kg) or
        the derivative with respect to P if deriv is set to True
    float or numpy array
        If deriv is set to True, the derivative of specific heat with respect
        to temperature
    ...
    if deriv:
        ...
        h_P += -762.587308746834 * LNG_PMPa
        h_T += -762.587308746834 * LNG_T
    ...
    LNG = np.tanh((-0.17506458742044 + -0.0787067450602815 * P_MPa + 0.0043943295092826 * T))
...
def lng_P(T, deriv=False):
    """
    Pressure of saturated LNG.
    This fit is from Eugina Mendez Ramos's thesis (http://hdl.handle.net/1853/64797).
    The equations are based on data obtained from the NIST chemistry webbook for
    saturated thermophysical properties of LNG. Fits were done in JMP software
    ...
    -------
    float or numpy array
       Pressure of saturated LNG (Pa) or the derivative with
...
def lng_h(T, deriv=False):
    ...
    -------
    float or numpy array
       Enthalpy of saturated LNG (J/kg) or the derivative with respect
...
def lng_u(T, deriv=False):
    """
    Internal energy of saturated LNG.
    This fit is from Eugina Mendez Ramos's thesis (http://hdl.handle.net/1853/64797).
    The equations are based on data obtained from the NIST chemistry webbook for
    saturated thermophysical properties of LNG. Fits were done in JMP software
    at temperatures ranging from 20.369-32 K. See page 100 of the thesis for more
    ...
    -------
    float or numpy array
       Internal energy of saturated LNG (J/kg) or the derivative with
       respect to T if deriv is set to True
    """
    if not isinstance(deriv, bool):
        raise ValueError("deriv input must be a boolean")
...
def lng_cp(T, deriv=False):
    """
    Specific heat at constant pressure of saturated LNG.
    This fit is from Eugina Mendez Ramos's thesis (http://hdl.handle.net/1853/64797).
    The equations are based on data obtained from the NIST chemistry webbook for
    saturated thermophysical properties of LNG. Fits were done in JMP software
    at temperatures ranging from 20.369-32 K. See page 100 of the thesis for more
    ...
    -------
    float or numpy array
       Specific heat at constant pressure of saturated LNG (J/(kg-K)) or the
...
def lng_rho(T, deriv=False):
    ...
    -------
    float or numpy array
       Density of saturated LNG (kg/m^3) or the derivative with respect
       to T if deriv is set to True
    """
    if not isinstance(deriv, bool):
        raise ValueError("deriv input must be a boolean")
...
def lng_k(T, deriv=False):
    """
    Thermal conductivity of saturated LNG.
    Uses a quadratic fit of the surrogate from LNG_properties.py.
    ...
    -------
    float or numpy array
        Thermal conductivity of saturated LNG (W/(m-K)) or
...
def lng_viscosity(T, deriv=False):
    """
    Dynamic viscosity of saturated LNG.
    Uses a quadratic fit of the surrogate from LNG_properties.py.
    This was mostly added for derivative testing purposes and not for production use.
    ...
    -------
    float or numpy array
        Dynamic viscosity of saturated LNG (Pa-s) or
        the derivative with respect to T if deriv is set to True
    """
    if not isinstance(deriv, bool):
        raise ValueError("deriv input must be a boolean")
...
def lng_beta(T, deriv=False):
    """
    Coefficient of thermal expansion of saturated LNG.
    Uses a quadratic fit of the surrogate from LNG_properties.py.
    This was mostly added for derivative testing purposes and not for production use.
    ...
    -------
    float or numpy array
        Coefficient of thermal expansion of saturated LNG (1 / K)
...
def sat_gng_rho(T, deriv=False):
    """
    Density of saturated gaseous natural gas.
    This fit is from Eugina Mendez Ramos's thesis (http://hdl.handle.net/1853/64797).
    The equations are based on data obtained from the NIST chemistry webbook for
    saturated thermophysical properties of LNG. Fits were done in JMP software
    at temperatures ranging from 20.369-32 K. See page 100 of the thesis for more
    ...
    -------
    float or numpy array
       Density of saturated gaseous natural gas (kg/m^3) or the derivative with respect
       to T if deriv is set to True
    """
    if not isinstance(deriv, bool):
        raise ValueError("deriv input must be a boolean")
...
def sat_gng_h(T, deriv=False):
    """
    Enthalpy of saturated gaseous natural gas.
    This fit is from Eugina Mendez Ramos's thesis (http://hdl.handle.net/1853/64797).
    The equations are based on data obtained from the NIST chemistry webbook for
    saturated thermophysical properties of LNG. Fits were done in JMP software
    at temperatures ranging from 20.369-32 K. See page 100 of the thesis for more
    ...
    -------
    float or numpy array
       Enthalpy of saturated gaseous natural gas (J/kg) or the derivative with respect
       to T if deriv is set to True
    """
    if not isinstance(deriv, bool):
        raise ValueError("deriv input must be a boolean")
...
def sat_gng_cp(T, deriv=False):
    """
    Specific heat at constant pressure of saturated gaseous natural gas.
    This fit is from Eugina Mendez Ramos's thesis (http://hdl.handle.net/1853/64797).
    The equations are based on data obtained from the NIST chemistry webbook for
    saturated thermophysical properties of LNG. Fits were done in JMP software
    at temperatures ranging from 20.369-32 K. See page 100 of the thesis for more
    ...
    -------
    float or numpy array
       Specific heat at constant pressure of saturated gaseous natural gas (J/(kg-K)) or
       the derivative with respect to T if deriv is set to True
    """
    if not isinstance(deriv, bool):
        raise ValueError("deriv input must be a boolean")
...
def sat_gng_k(T, deriv=False):
    """
    Thermal conductivity of saturated gaseous natural gas.
    This fit is from Eugina Mendez Ramos's thesis (http://hdl.handle.net/1853/64797).
    The equations are based on data obtained from the NIST chemistry webbook for
    saturated thermophysical properties of LNG. Fits were done in JMP software
    at temperatures ranging from 20.369-32 K. See page 100 of the thesis for more
    ...
    -------
    float or numpy array
       Thermal conductivity of saturated gaseous natural gas (W/(m-K)) or the derivative
...
def sat_gng_viscosity(T, deriv=False):
    """
    Viscosity of saturated gaseous natural gas.
    This fit is from Eugina Mendez Ramos's thesis (http://hdl.handle.net/1853/64797).
    The equations are based on data obtained from the NIST chemistry webbook for
    saturated thermophysical properties of LNG. Fits were done in JMP software
    at temperatures ranging from 20.369-32 K. See page 100 of the thesis for more
    ...
    -------
    float or numpy array
       Viscosity of saturated gaseous natural gas (Pa-s) or the derivative with respect
       to T if deriv is set to True
    """
    if not isinstance(deriv, bool):
        raise ValueError("deriv input must be a boolean")
...
def sat_gng_beta(T, deriv=False):
    """
    Coefficient of thermal expansion of saturated gaseous natural gas.
    This fit is from Eugina Mendez Ramos's thesis (http://hdl.handle.net/1853/64797).
    The equations are based on data obtained from the NIST chemistry webbook for
    saturated thermophysical properties of LNG. Fits were done in JMP software
    at temperatures ranging from 20.369-32 K. See page 100 of the thesis for more
    ...
    -------
    float or numpy array
       Coefficient of thermal expansion of saturated gaseous natural gas (1 / K) or the
       derivative with respect to T if deriv is set to True
    """
    if not isinstance(deriv, bool):
        raise ValueError("deriv input must be a boolean")
...
def sat_gng_T(P, deriv=False):
    """
    Temperature of saturated gaseous natural gas at the specified pressure.
    This fit is from Eugina Mendez Ramos's thesis (http://hdl.handle.net/1853/64797).
    The equations are based on data obtained from the NIST chemistry webbook for
    saturated thermophysical properties of LNG. Fits were done in JMP software
    at temperatures ranging from 20.369-32 K. See page 100 of the thesis for more
    ...
    -------
    float or numpy array
       Temperature of saturated gaseous natural gas (K) or the derivative with respect
